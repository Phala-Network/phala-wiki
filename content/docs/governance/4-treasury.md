...
The Treasury is a pot of funds collected through transaction fees, slashing, Secure Worker mining rate (20%) , etc. The funds held in the Treasury can be spent by making a spending proposal that, if approved by the [Council](https://wiki.polkadot.network/docs/learn-governance#council), will enter a waiting period before distribution. This waiting period is known as the budget period, and its duration is subject to [governance](https://wiki.polkadot.network/docs/learn-governance), with the current default set to 1 day. The Treasury attempts to spend as many proposals in the queue as it can without running out of funds.
...
The Treasury is ultimately controlled by the [Council](https://wiki.polkadot.network/docs/learn-governance#council), and how the funds will be spent is up to their judgment.
